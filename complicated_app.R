  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Plots", 
                 fluidRow(column(4,
                                 selectInput('wy', label = "Select a water year to analze", 
                                             choices = 2010:2019)),
                          column(4,
                                 textOutput("site")),
                          column(4,
                                 actionButton("dl_snotel", "Download SNOTEL Data for Water Year Selected")),
                          column(4,
                                 actionButton("dl_asos", "Download ASOS Data for Water Year Selected"))),
...
server <- function(input, output, session) {
  ...
  output$snotelmap <- renderLeaflet({
    leaflet() %>%
      ...
      addPolygons(data = snotel_in_hucs,
                 color = "red",
                 popup = snotel_in_hucs$Name,
                 layerId = snotel_in_hucs$HUC8,
                 group = "snotel")
...
#}) #end asos map  
  ...
  snotel_id <- reactive({
    ...
    (input$snotelmap_shape_click$id)
  ...
  snotel_site <- reactive({
    snotel_spatial %>%
      filter(site_no == snotel_id()$id)
  })
  
  
  output$site <- renderText({
    paste(snotel_site()$site_name,input$wy)
  })
 ...
 output$snotel_temp <-  renderDygraph({
   ...
   sno_ts = xts(df$avg_temp_f,
                order.by=df$date)
   dygraph(sno_ts,  main = "Temperature",group='graphs') %>%
     dySeries(color = "red") %>%
